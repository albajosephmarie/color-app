export const colorAppSvgDownload = (palette, darkModePalette) => {
  ...
  const downloadSvg = (title) => {
    const svgContent = `
    <svg width="${svgWidth}" height="${svgHeight + svgDarkModeHeight + 30
    ...
    ${palette
      ...
      <rect x="${(index % squaresPerLine) * (squareWidth + margin) + margin}" 
            y="${Math.floor(index / squaresPerLine) * (squareHeight + margin) +
            titleHeight
            }" 
            width="${squareWidth}" 
            height="150" 
      ...
      <text x="${(index % squaresPerLine) * (squareWidth + margin) + margin + 80
            }" 
            y="${Math.floor(index / squaresPerLine) * (squareHeight + margin) +
            titleHeight +
            36
      ...
      <rect x="${(index % squaresPerLine) * (squareWidth + margin) + margin + 90
            }" 
            y="${Math.floor(index / squaresPerLine) * (squareHeight + margin) +
            titleHeight +
            16
      ...
      <text x="${(index % squaresPerLine) * (squareWidth + margin) + margin + 100
            }" 
            y="${Math.floor(index / squaresPerLine) * (squareHeight + margin) +
            titleHeight +
            36
      ...
      <text x="${(index % squaresPerLine) * (squareWidth + margin) + margin + 25
            }" 
            y="${Math.floor(index / squaresPerLine) * (squareHeight + margin) +
            titleHeight +
            110
        ...
        <text x="${(index % squaresPerLine) * (squareWidth + margin) + margin + 120
            }" 
            y="${Math.floor(index / squaresPerLine) * (squareHeight + margin) +
            titleHeight +
            110
    ...
    `
        ...
        .join("")}
    <text x="10" y="${svgHeight + 30
      }" font-size="24" font-weight="bold">Dark Mode</text>
    ${darkModePalette
        .map(
          (e, index) => `
      <rect x="${(index % squaresPerLine) * (squareWidth + margin) + margin}" 
          y="${svgHeight +Math.floor(index / squaresPerLine) * (squareHeight + margin) +
          titleHeight
          }" 
          width="${squareWidth}" 
          height="150" 
